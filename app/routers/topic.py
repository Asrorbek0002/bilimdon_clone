...
async def create_topic(topic: TopicCreate, db: db_dep, admin_user: admin_user_dep):
    if db.query(Topic).filter(Topic.name == topic.name):
...
async def delete_topic(id:int, db: db_dep, admin_user: admin_user_dep):
    ...
    return {"topic_id": id, "message": "Topic deteled"}
