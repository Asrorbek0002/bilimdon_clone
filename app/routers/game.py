...
from app.schemas.game import GameCreate, GameUpdate,GameResponse, GameSelectQuestion
...
async def get_games(db: db_dep):
    return db.query(Game).filter(Game.end_time > datetime.now(timezone.utc))
...
async def create_game(game: GameCreate, db: db_dep, current_user: current_user_dep):
    ...
    if game.end_time - game.start_time < timedelta(hours=1):
        raise HTTPException(status_code=400, detail="Game must last at least 1 hour.")
...
async def update_game(id: int, game: GameUpdate, db: db_dep):
...
async def delete_game(id: int, db: db_dep, admin_user: admin_user_dep):
    db_game = db.query(Game).filter(Game.id == id).first()
    ...
    return {"game_id": id, "message": "Game deleted"}

@router.get("/{id}/questions", response_model=list[QuestionResponse])
async def game_question(db: db_dep, current_user: current_user_dep, id: int):
    db_game = db.query(Game).filter(Game.id == id).first()
    if not db_game:
        raise HTTPException(status_code=404, detail="Game not found")
    return []
...
async def select_question(db: db_dep, current_user: current_user_dep, request: GameSelectQuestion):
    ...
    db_game_question = GameQuestion(game_id = db_question.id, question_id = db_question.id)
