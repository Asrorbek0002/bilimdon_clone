...
from app.schemas.option import OptionUpdate, OptionCreate, OptionResponse

router = APIRouter(prefix="/options", tags=["options"])

@router.get("/", response_model=list[OptionResponse])
async def get_option(db: db_dep):
...
async def create_option(option: OptionCreate, db: db_dep, current_user: current_user_dep):
    ...
    if existing_correct_option and option.is_correct:
        raise HTTPException(status_code=400, detail="Question has already a correct option")
...
async def delete_option(id: int, db: db_dep):
    ...
    return {"option_id":id, "message": "Option delete."}
